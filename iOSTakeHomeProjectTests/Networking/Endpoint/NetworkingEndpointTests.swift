...
//
//  Created by Joao Gripp on 26/07/23.
//
...
final class NetworkingEndpointTests: XCTestCase {
    ...
    func test_with_people_endpoint_request_is_valid() {
        ...
        XCTAssertEqual(endpoint.url?.absoluteString, "https://reqres.in/api/users?page=1&delay=8", "The generate does not match our endpoint" )
    ...
    func test_with_detail_endpoint_request_is_valid() {
        ...
        XCTAssertEqual(endpoint.url?.absoluteString, "https://reqres.in/api/users/\(userId)?delay=8", "The generate does not match our endpoint" )
    }
    
    func test_with_create_endpoint_is_valid() {
        ...
        XCTAssertEqual(endpoint.url?.absoluteString, "https://reqres.in/api/users?delay=8", "The generate does not match our endpoint" )
    }
}
