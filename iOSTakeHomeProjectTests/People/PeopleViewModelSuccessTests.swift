...
final class PeopleViewModelSuccessTests: XCTestCase {
    ...
    func test_with_successful_paginated_response_users_array_is_set() async throws {
        ...
        defer {
            XCTAssertFalse(vm.isFetching, "The View model should not be fetching any data")
            XCTAssertEqual(vm.viewState, .finished, "The view model state should be finished")
        }
    ...
    func test_with_last_user_func_returns_true() async {
        ...
        let usersData = try! StaticJSONMapper.decode(file: "UsersStaticData", type: UsersResponse.self)
        
        let hasReachedEnd = vm.hasReachedEnd(of: usersData.data.last!)
